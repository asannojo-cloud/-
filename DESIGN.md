...
/member/card               로그인 후 첫 화면 = 모바일 회원증 (기본 진입점)
...
**배경**: 실제 조합원 규모가 약 1,900명이며, 모바일 회원증 링크를 일괄 발송한 뒤 각자 첫 로그인을 하는 운영 방식이 확정되었다. 회원번호(비정형 값)와 관리자 발급 임시 비밀번호를 1,900명에게 개별 전달하는 것은 운영 부담이 크므로, 각 회원이 이미 알고 있는 **이름 + 본인 휴대폰번호**로 로그인하도록 변경했다.
